...
name="Vicky"
app = Flask(__name__)
app.secret_key = '4x7KiqWXK9$mYpV2'  # Secure random key for session management
...
def init_db():
    ...
    # Import existing attendance from CSV if available
    try:
...
def data():
    ...
    if request.method=="POST":
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT DISTINCT NAME, Time, Date FROM Attendance WHERE Date=?", (str(today),))
...
def whole():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT DISTINCT NAME, Time, Date FROM Attendance")
        rows = cursor.fetchall()
...
def dashboard():
    ...
    try:
        ...
        # Get recent records
        cursor.execute("SELECT NAME, Time, Date FROM Attendance ORDER BY Date DESC, Time DESC LIMIT 10")
        ...
        cursor.execute("SELECT COUNT(DISTINCT NAME) FROM Attendance WHERE Date=?", (str(today),))
        ...
        for i in range(6, -1, -1):
            ...
            cursor.execute("SELECT COUNT(DISTINCT NAME) FROM Attendance WHERE Date=?", (day_str,))
        ...
        cursor.execute("""
            ...
            LIMIT 5
        """)
        top_attendees_data = cursor.fetchall()
        top_attendees = [{'name': row[0], 'count': row[1]} for row in top_attendees_data]
        ...
        cursor.execute("""
            ...
            ORDER BY hour
        """, (str(today),))
        time_dist_data = cursor.fetchall()
        
        time_labels = []
        time_values = []
        
        for row in time_dist_data:
            time_labels.append(f"{row[0]}:00")
            ...
            for i, name in enumerate(student_names):
                if i < len(colors):
                    ...
                    for day in trend_dates:
                        ...
                        cursor.execute("SELECT COUNT(*) FROM Attendance WHERE NAME=? AND Date=?", 
                                    (name, day_date))
                        count = cursor.fetchone()[0]
...
def markData(name):
    try:
        ...
        try:
            ...
            cursor.execute("SELECT COUNT(*) FROM Attendance WHERE NAME=? AND Date=?", (name, str(today)))
            count = cursor.fetchone()[0]
            
            if count == 0:
                cursor.execute("INSERT INTO Attendance (NAME, Time, Date) VALUES (?, ?, ?)",
                            (name, dtString, str(today)))
...
def recognize_and_mark():
    ...
    try:
        ...
        stream_url = "http://192.168.171.210:8080/?action=stream"
...
if __name__ == '__main__':
    ...
    # Enable keep-alive functionality in production to prevent spin-down
